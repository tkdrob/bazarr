...
def update_movies():
    ...
    if apikey_radarr is None:
        ...
        try:
            r = requests.get(url_radarr_api_movies, timeout=int(settings.radarr.radarr_timeout), verify=False)
            r.raise_for_status()
...
def get_profile_list():
    ...
    try:
        profiles_json = requests.get(url_radarr_api_movies, timeout=int(settings.radarr.radarr_timeout), verify=False)
    except requests.exceptions.ConnectionError as errc:
