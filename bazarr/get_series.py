...
def update_series():
    ...
    if apikey_sonarr is None:
        ...
        try:
            r = requests.get(url_sonarr_api_series, timeout=int(settings.sonarr.sonarr_timeout), verify=False)
            r.raise_for_status()
...
def get_profile_list():
    ...
    try:
        profiles_json = requests.get(url_sonarr_api_series, timeout=int(settings.sonarr.sonarr_timeout), verify=False)
    except requests.exceptions.ConnectionError as errc:
        error = True
    ...
    try:
        profiles_json_v3 = requests.get(url_sonarr_api_series_v3, timeout=int(settings.sonarr.sonarr_timeout), verify=False)
    except requests.exceptions.ConnectionError as errc:
        error = True
        logging.exception("BAZARR Error trying to get profiles from Sonarr. Connection Error.")
    except requests.exceptions.Timeout as errt:
